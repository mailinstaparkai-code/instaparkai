...
| 0.5.6 | 18 | 2026-09-01 | `releases/instapark-valet-v0.5.6-build18.apk` | OCR-assisted plate number at check-in, reusing the same server-side Tesseract.js engine and "suggestion, not truth" philosophy already used for DL-expiry OCR during operator onboarding (`lib/dl-ocr.ts` → new sibling `lib/plate-ocr.ts`). Both web and Android check-in dialogs gain an optional "Plate" photo slot next to the existing Front/Back/Left/Right/Odometer ones (`valet_tickets.check_in_photos` already supports arbitrary named slots, zero schema change); capturing a photo there runs it through OCR and pre-fills the vehicle-number field with an "OCR suggested — please verify" hint, fully editable/overridable, and skipping it entirely still allows a normal manual check-in exactly as before. New `POST v1/queue/ocr-plate` endpoint (`requireApiSession` only, no DB access) backs the Android side; `checkIn`'s multipart payload gains a 6th optional `photo_plate` part. Along the way, found and fixed a real, previously-dormant bug that also affects the existing DL-expiry OCR feature: `tesseract.js` isn't on Next's default `serverExternalPackages` list, so Turbopack was bundling it and mangling the `require()` it uses internally to locate its own worker-script file at runtime — every OCR call was crashing with `Cannot find module '.../tesseract.js/src/worker-script/node/index.js'` the moment it actually ran. Fixed by adding `serverExternalPackages: ["tesseract.js"]` to `next.config.ts` (`apps/super-admin`) — this is a web-only fix with no Android-side change, but it means the operator DL-expiry OCR flow was likely never functional in production until this release. Same signing identity as 0.5.5 (verified via `apksigner verify`, identical SHA-256 digest). Verified end-to-end against the local dev server (pointed at the same production-linked DB): after the config fix, `curl`-tested the new endpoint with a real synthetic plate photo (correct read, `KA01AB1234`), a blank non-plate image (graceful `null`, no error), and a full check-in with the plate photo attached (confirmed `photo_plate` persisted correctly in `check_in_photos` and cleaned up afterward); also confirmed skipping the photo still allows a plain manual check-in (regression check). Kotlin compiles clean (`compileDebugKotlin`, `assembleRelease`). Did **not** get a live on-device/browser click-through of the new Plate capture tile and its OCR pre-fill this pass — browser-based login to the parking-admin test account is blocked by this session's own safety policy on entering passwords into web forms, so this release's UI wiring (`plate-capture-field.tsx`, `QueueScreen.kt`'s `CheckInDialog`) is code-reviewed and backend-proven rather than device-confirmed; flag any plate-scan UI glitch for a quick follow-up fix. |
