...
import { listZones, createZone, mapZoneForApi } from "@/lib/parking-admin/configuration";

export async function GET(req: Request) {
  try {
    const session = await requireApiSession(req);
    requireApiParkingAdmin(session);
    const zones = await listZones(createServiceClient(), getCurrentSiteId(session));
    return NextResponse.json({ zones: zones.map(mapZoneForApi) });
  } catch (err) {
...
export async function POST(req: Request) {
  try {
    ...
    const zone = await createZone(createServiceClient(), getCurrentSiteId(session), name);
