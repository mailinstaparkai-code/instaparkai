...
// Vendored (see ../vendor/) rather than required as bare package specifiers: this
// file is only ever reached via a runtime path string, invisible to the build's
// file tracer, so plain node_modules dependencies here would silently go missing
// from the deployed function exactly like the rest of this worker script did.
require(path.join(__dirname, '..', 'vendor', 'regenerator-runtime', 'runtime.js'));
const isURL = require(path.join(__dirname, '..', 'vendor', 'is-url', 'index.js'));
const dump = require(path.join(__dirname, 'utils', 'dump.js'));
const env = require(path.join(__dirname, '..', 'utils', 'getEnvironment.js'))('type');
const setImage = require(path.join(__dirname, 'utils', 'setImage.js'));
const defaultOutput = require(path.join(__dirname, 'constants', 'defaultOutput.js'));
const { log, setLogging } = require(path.join(__dirname, '..', 'utils', 'log.js'));
const PSM = require(path.join(__dirname, '..', 'constants', 'PSM.js'));
